 ...
 *
 * @param {Object} opts
 * @param {String} opts.coin - default 'cmm'
...
API.prototype.seedFromRandom = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.create(opts.coin || 'cmm', opts.network || 'livenet');
};
...
API.prototype.seedFromRandomWithMnemonic = function(opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.createWithMnemonic(opts.coin || 'cmm', opts.network || 'livenet', opts.passphrase, opts.language || 'en', opts.account || 0);
};
...
API.prototype.seedFromExtendedPrivateKey = function(xPrivKey, opts) {
  opts = opts || {};
  this.credentials = Credentials.fromExtendedPrivateKey(opts.coin || 'cmm', xPrivKey, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts);
};
...
API.prototype.seedFromMnemonic = function(words, opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.fromMnemonic(opts.coin || 'cmm', opts.network || 'livenet', words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts);
};
...
API.prototype.seedFromExtendedPublicKey = function(xPubKey, source, entropySourceHex, opts) {
  ...
  opts = opts || {};
  this.credentials = Credentials.fromExtendedPublicKey(opts.coin || 'cmm', xPubKey, source, entropySourceHex, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44);
};
...
API.prototype.importFromMnemonic = function(words, opts, cb) {
  ...
  function derive(nonCompliantDerivation) {
    return Credentials.fromMnemonic(opts.coin || 'cmm', opts.network || 'livenet', words, opts.passphrase, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, {
...
API.prototype.importFromExtendedPublicKey = function(xPubKey, source, entropySourceHex, opts, cb) {
  ...
  log.debug('Importing from Extended Private Key');
  try {
    this.credentials = Credentials.fromExtendedPublicKey(opts.coin || 'cmm', xPubKey, source, entropySourceHex, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, opts);
  } catch (e) {
    log.info('xPriv error:', e);
    return cb(new Errors.INVALID_BACKUP);
  };
 ...
 *
 * @param {string} coin - 'cmm' (default) or 'bch'
...
API.prototype.getFeeLevels = function(coin, network, cb) {
  ...
  $.checkArgument(coin || _.includes(['cmm', 'bch'], coin));
  $.checkArgument(network || _.includes(['livenet', 'testnet'], network));

  self._doGetRequest('/v2/feelevels/?coin=' + (coin || 'cmm') + '&network=' + (network || 'livenet'), function(err, result) {
    if (err) return cb(err);
 ...
 * @param {string} opts.coin[='cmm'] - The coin for this wallet (btcz, bch).
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  var coin = opts.coin || 'cmm';
  if (!_.includes(['cmm', 'bch'], coin)) return cb(new Error('Invalid coin'));
 ...
 * @param {String} copayerName
 * @param {Object} opts
 * @param {string} opts.coin[='cmm'] - The expected coin for this wallet (btcz, bch).
...
API.prototype.getBalance = function(opts, cb) {
  ...
  if (opts.coin) {
    if (!_.includes(['cmm', 'bch'], opts.coin)) return cb(new Error('Invalid coin'));
